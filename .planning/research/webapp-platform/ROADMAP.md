...
- **Label format — critical finding, verified against live Ultralytics source, not assumed:** every label line is `<class-index> <x1> <y1> <x2> <y2> ... <xn> <yn>` — **normalized polygon coordinates (YOLO segmentation format)**, not the simpler 5-value `<class-index> <x_center> <y_center> <width> <height>` detection format. One inspected line has 83 polygon points (166 coordinate values) for a single `car` instance. This directly and concretely answers one of the open questions in the original brief (§12 "whether standard YOLO detection labels support arbitrary polygons"): **they do not** — polygon annotation requires the distinct segmentation label format and (per YOLO-FORMAT-AND-API.md §1) a segmentation-task model checkpoint and dataset-wide consistency. See §7 and §9 below for the full implications.
...
| 10 | **Expected concurrent users** | Single-operator tool vs. small-team multi-user | **Not determined by this research — must be answered by the project owner before Phase 3 is finalized** | Directly determines DB engine choice (row 6) and whether any authentication/authorization layer is needed at all (§15) | **Yes** — blocks finalizing Phase 3 |
